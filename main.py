...
def download_playlist_songs(playlist_url, ffmpeg_path):
    ...
    for video in playlist.videos:
        file_name = video.title.replace("|", "").replace(":", "").replace('"', "").replace("/", "").replace("\\", "").replace("?", "").replace("*", "")
        stream = video.streams.get_audio_only()
        file_path = stream.download(output_path=download_folder, filename=f"{file_name}.mp4")
